        ...
        if not self.loaded:
            ...
            if self.db_alias not in connections:
                raise ImproperlyConfigured(
                    ...
                    "You should either set YAMDL_DATABASE_ALIAS or add a DATABASES entry." % self.db_alias
                )
            self.connection = connections[self.db_alias]
            # Make sure it's shared in-memory SQLite
            if self.connection.vendor != "sqlite":
                raise ImproperlyConfigured("The Yamdl database must be shared in-memory SQLite")
            ...
            if not hasattr(settings, "YAMDL_DIRECTORIES"):
                raise ImproperlyConfigured("You have not set YAMDL_DIRECTORIES to a list of directories to scan")
            for directory in settings.YAMDL_DIRECTORIES:
                if not os.path.isdir(directory):
                    raise ImproperlyConfigured("Yamdl directory %s does not exist" % directory)
